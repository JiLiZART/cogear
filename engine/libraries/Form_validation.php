...
|---------------------------------------------------------------
| Purpose:
| Only change original Language class method to i18n gear methods
|===============================================================
*/
class CI_Form_validation {
	...
	function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		if (is_null($postdata) AND $callback == FALSE)
		{
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules))
			{
				...
				{
					if (FALSE === ($line = t('!form_validation '.$type)))
					{
		...
		foreach ($rules As $rule)
		{
			...
			{				
				if ( ! method_exists($this, $rule))
				{
					...
					if (function_exists($rule))
					{
						...
						{
							$this->_field_data[$row['field']]['postdata'] = (is_bool($result) && !$result) ? $result : $postdata;
						}
					}
			...
			{			
				...
				{
					if (FALSE === ($line = t('!form_validation '.$rule)))
					{
				...
				// Changed by Dmitry Belyaev to correctly show matches field label 22.12.2008
				// Build the error message
				$message = sprintf($line, $this->_translate_fieldname($row['label']), isset($this->_field_data[$param]) ? (isset($this->_field_data[$param]['label']) ? $this->_field_data[$param]['label'] : $param): $param);
	...
	function _translate_fieldname($fieldname)
	{
		...
		if (substr($fieldname, 0, 5) == 'lang:')
		{
			...
			if (FALSE === ($fieldname = t('!form_validation '.$line)))
			{
				return $line;
			}
		}
