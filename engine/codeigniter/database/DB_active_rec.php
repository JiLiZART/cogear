...
|---------------------------------------------------------------
| Purpose:
| Add second argument which allows not to erase current query after counting all results
| For better perfomance replace "COUNT (*)" to "COUNT (table_name.id)"
|===============================================================
*/
   ...
   * @access  public
   * @param string
   * @param boolean - erase query after count or not // CoGear, Dmitry Belyaev
   * @param boolean - use fast count by id param
   * @return  string
   */
  function count_all_results($table = '',$erase_params = TRUE, $fast_count = TRUE)
  {
    if ($table != '')
    {
      if ($this->dbprefix != '') {$table = $this->dbprefix.$table;}
      $this->_track_aliases($table);
      $this->from($table);
      $count_string = str_replace('*',$table.'.id',$this->_count_string);
    }
    else {
      $count_string = $this->_count_string;
    }
  
    $sql = $this->_compile_select($count_string. $this->_protect_identifiers('numrows'));

    $query = $this->query($sql);
    $this->_reset_select($erase_params); // <= There we use new argument
...
|---------------------------------------------------------------
| Purpose:
| Additional functionality for method allow it not to reset all query params but only 'from' query
|===============================================================
*/
   ...
   * @return  void
   */
  function _reset_select($erase_params = TRUE)
  {
    if($erase_params === TRUE){
    ...
    elseif(!$erase_params) {
      ...
      $erase_params = (array)explode(',',$erase_params);
      $ar_reset_items = array(
                  'ar_select'     => array(), 
                  'ar_from'     => array(), 
                  'ar_join'     => array(), 
      ...
      foreach($erase_params as $param){
        foreach($ar_reset_items as $key=>$item){
          if(strpos($key,$param) !== FALSE) unset($ar_reset_items[$key]);
        }
      }
    }
...
*/
  ...
  /**
   * Swop active records params.
   *
   * @access  public
   * @param mixed
   * @return  void
   */
  function swop($exclude = FALSE){
    $ar_items = array(
                  'ar_set'    => array(), 
                  'ar_select'     => array(), 
                  'ar_from'     => array(), 
    ...
    if($this->ar_swop){
      foreach($this->ar_swop as $key=>$value){
        $this->$key = $value;
      }
      $this->ar_swop = FALSE;
    }
    else {
      foreach($ar_items as $key=>$value){
        $this->ar_swop[$key] = $this->$key;
      }
      $this->_reset_run($ar_items);
    }
    return $this;
  }
