...
class Unzip{
    ...
    Function unzipFile($compressedFileName, $targetFileName=false){
        ...
        if($this->applyChmod && $targetFileName){
			// Dmitry Belyaev - added another chmod form config files
			if(in_array(pathinfo($targetFileName,PATHINFO_EXTENSION),array('info'))){
				@chmod($targetFileName, 0777);
			}
            else chmod($targetFileName, 0644);
        }
    ...
    Function unzipAll(){
        ...
        if(sizeof($lista))
          foreach($lista as $file=>$trash){
            ...
            if(substr($dirname, 0, strlen($this->baseDir)) != $this->baseDir OR strpos($dirname,'.') === 0 && $dirname != '.' OR strpos($dirname,'_') === 0)
                continue;
            ...
            if(!is_dir($outDN) && $this->maintainStructure){
                ...
                foreach($folders as $folder){
                    ...
                    if(!is_dir($this->targetDir."/".$str)){
                        ...
                        @mkdir($this->targetDir."/".$str, 0777,TRUE);
                        if($this->applyChmod)
