...
var FormCheck = new Class({
	...
	options : {
		...
		errorClass: 'error',				//div error class
		...
		el.validation.each(function(rule) {
			if(this.isChildType(el)) {
				...
				if(rule.match(/^.+\[/)) { 
					...
					ruleArgs = rule.match(/^.+\[(.+)\]$/)[1].split(",")
					// Changed by Dmitry Belyaev in purpose to work normally with file ext 09.12.2008
					//eval(rule.match(/^.+(\[.+\])$/)[1].replace(/([A-Z\._]+)/i, "'$1'"));
	...
	*/
	validateRegex : function(el, ruleMethod, ruleArgs) {
		...
		} // Extended by Dmitry Belyaev 09.12.2008
		else if(ruleArgs[0] && ruleMethod == 'file'){
		 this.options.regexp.file = new RegExp(".+\.("+ruleArgs.join("|")+")$");
		 msg = this.options.alerts.file.replace("%ext",ruleArgs.join(", "));
		}
		else {
	...
	*/
	validateConfirm: function(el,ruleArgs) {
		...
		if(el.value != this.form[confirm].value){
			...
			else if(this.options.display.labelsInsteadNames){
				var msg = this.options.alerts.confirm.replace("%0",this.form[confirm].getParent().getElement('label').get('text'));
			}	
			else
	...
	validateDiffers: function(el,ruleArgs) {
		...
		if(el.value == this.form[differs].value){
			if (this.options.display.titlesInsteadNames)
				...
				var msg = this.options.alerts.confirm.replace("%0",this.form[differs].getParent().getElement('label').get('text'));
			}	
			else
	...
	addError : function(obj) {
		if(!obj.element && this.options.display.indicateErrors != 0) {
			if (this.options.display.errorsLocation == 1) {
				...
				obj.element.setStyle('width',obj.getStyle('width'));
			...
			if(obj.get('type') != 'file' && !Browser.Engine.presto) {
				var error_fields = $$('form .'+this.options.errorClass);				
				 if(error_fields.length > 0){
						var border = '1px solid '+error_fields[0].getStyle('border').split(' ')[2];
				 }
				 else {
						var border = '1px solid '+obj.element.getStyle('color');
				 }
				obj.setStyle('border',this.options.err_border || border);
			}
		}					
		if (obj.element) {
			obj.element.empty();
			...
			var tab_menu = $$('ul.tab-menu');
			if(tab_menu.length > 0){
				tab_menu.getChildren().each(function(tab,i){
				var tab_wrapper = tab_menu.getParent().getParent().getNext();
					tab_wrapper.getChildren().each(function(child,i){
						if(child.hasChild(obj.id)){
							tab.fireEvent('click');
						}
					});
				});
			...
			if(this.options.msg && obj.getParent()) {
				msg(obj.errors.join('<br>'),obj.getParent().getElement('label') ? obj.getParent().getElement('label').get('text').replace(/(\s)+/g,'').replace('*','') : obj.name);
			}
	...
	*/
	removeError : function(obj) {
		...
		obj.errors = [];
		obj.isOK = true;
		if(this.options.border) obj.setStyle('border',this.options.border);
